...
static int digit()
{
	int reg; //stores the result of parsing and evaluating token

	if (!is_digit(token)) {
		ERROR("Expected digit\n");		//error if it is not 0 | 1 | .... | 9
		exit(EXIT_FAILURE);
	}
...
static int variable()
{
	int reg; //stores result
	if (!is_identifier(token))
	{
		ERROR("Expected character between a and f\n"); //error if not a,b,c,d,e or f
		exit(EXIT_FAILURE);
	}
	reg = next_register(); //get next available register
...
static void print()
{
	...
	if (token != '%')
	{
		ERROR("Expected '%' for print statement\n");
        exit(EXIT_FAILURE);
	}
	next_token();
	char var = token;
